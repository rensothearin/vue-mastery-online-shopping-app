  ...
  <header class="bg-white">
    <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
      ...
      <PopoverGroup class="hidden lg:flex lg:gap-x-12">
        <Popover class="relative">
          ...
          <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="translate-y-0" leave-to-class="opacity-0 translate-y-1">
            <PopoverPanel class="absolute left-1/2 z-10 mt-3 w-screen max-w-md -translate-x-1/2 overflow-hidden rounded-3xl bg-white shadow-lg outline outline-1 outline-gray-900/5">
              <div class="p-4">
      ...
      <div class="hidden lg:flex lg:flex-1 lg:justify-end lg:gap-x-12">
        <button type="button" @click="cartOpen = !cartOpen" class="relative -m-2.5 p-2.5 text-gray-700" aria-label="Open cart">
            <ShoppingBagIcon class="size-6" aria-hidden="true" />
            <span v-if="itemCount > 0" class="absolute -top-1 -right-1 inline-flex items-center justify-center rounded-full bg-red-600 text-white text-xs font-semibold leading-none h-5 min-w-[18px] px-1.5">
                {{ itemCount }}
            </span>
        </button>

        <div v-if="cartOpen" @click.outside="cartOpen = false" class="absolute right-0 z-50 mt-12 w-80 rounded bg-white shadow-lg ring-1 ring-black/5">
            <div class="p-4">
              <h4 class="text-sm font-semibold mb-2">Cart</h4>
              ...
              <ul v-else class="space-y-3 max-h-72 overflow-auto">
                <li v-for="(it, i) in state.items" :key="i" class="flex items-start gap-3">
                  <img :src="it.image" alt="" class="size-12 rounded object-cover" />
                  <div class="flex-auto">
                    <h5 class="text-sm font-medium text-gray-900">{{ it.name }}</h5>
                    <p class="text-sm text-gray-500">Qty: {{ it.qty }}</p>
                    <p class="text-sm font-semibold text-gray-900 mt-1">{{ it.price }}</p>
                    </div>
                </li>
                </ul>
                <div v-if="state.items.length > 0" class="mt-4">
