    ...
    <div class="pt-6">
      <nav aria-label="Breadcrumb">
        <ol role="list" class="mx-auto flex max-w-2xl items-center space-x-2 px-4 sm:px-6 lg:max-w-7xl lg:px-8">
          <li v-for="breadcrumb in womenProduct.breadcrumbs" :key="breadcrumb.id">
          ...
          <li class="text-sm">
            <a :href="womenProduct.href" aria-current="page" class="font-medium text-gray-500 hover:text-gray-600">{{ womenProduct.name }}</a>
          </li>
        </ol>
      </nav>
      ...
      <div class="mx-auto max-w-2xl px-4 pb-16 pt-10 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto_auto_1fr] lg:gap-x-8 lg:px-8 lg:pb-24 lg:pt-16">
        <div class="lg:col-span-2 lg:border-r lg:border-gray-200 lg:pr-8">
          <h1 class="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">{{ womenProduct.name }}</h1>
        </div>

        <!-- Options -->
        <div class="mt-4 lg:row-span-3 lg:mt-0">
          <h2 class="sr-only">Product information</h2>
          <p class="text-3xl tracking-tight text-gray-900">{{ womenProduct.price }}</p>
          ...
          <form class="mt-10" @submit.prevent="addToCart">
            ...
            <div>
              ...
              <fieldset aria-label="Choose a color" class="mt-4">
                <div class="flex items-center gap-x-3">
                  <div v-for="color in womenProduct.colors" :key="color.id" class="flex rounded-full outline outline-1 -outline-offset-1 outline-black/10">
                    <input 
            ...
            <div class="mt-10">
              ...
              <fieldset aria-label="Choose a size" class="mt-4">
                <div class="grid grid-cols-4 gap-3">
                  <label
                    v-for="size in womenProduct.sizes"
                    :key="size.name"
        ...
        <div class="py-10 lg:col-span-2 lg:col-start-1 lg:border-r lg:border-gray-200 lg:pb-16 lg:pr-8 lg:pt-6">
          ...
          <div>
            ...
            <div class="space-y-6">
              <p class="text-base text-gray-900">{{ womenProduct.description }}</p>
            </div>
          </div>
          ...
          <div class="mt-10">
            ...
            <div class="mt-4">
              <ul role="list" class="list-disc space-y-2 pl-4 text-sm">
                <li v-for="highlight in womenProduct.highlights" :key="highlight" class="text-gray-400">
          ...
          <div class="mt-10">
            ...
            <div class="mt-4 space-y-6">
              <p class="text-sm text-gray-600">{{ womenProduct.details }}</p>
            </div>
          </div>
        </div>
      </div>
...
const selectedColor = ref(womenProduct.value.colors[0].id)
const colorImages = {
    mixed: womenProduct.value.images[0],
    black: womenProduct.value.images[1],
    gray: womenProduct.value.images[2],
    white: womenProduct.value.images[3],
}
const displayedImages = computed(() => {
  const mainImage = colorImages[selectedColor.value];
  if (!mainImage) {
    return womenProduct.value.images; // Fallback to default images
  }
  // Return the selected color's image first, followed by other images, ensuring no duplicates.
  return [mainImage, ...womenProduct.value.images.filter(img => img.src !== mainImage.src)];
})

const firstInStock = womenProduct.value.sizes.find(size => size.inStock)
const selectedSize = ref(firstInStock ? firstInStock.name : womenProduct.value.sizes[0].name)

function addToCart() {
  const p = womenProduct.value
  if (!p) return
    addItem({
        name: womenProduct.value.name,
        price: womenProduct.value.price,
