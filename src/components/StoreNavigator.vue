          ...
          <div class="flex h-16 items-center">
            ...
            <div class="ml-auto flex items-center">
              ...
              <div class="ml-4 flow-root lg:ml-6">
                <a href="#" class="group relative -m-2 flex items-center p-2" @click="cartOpen = true">
                  <ShoppingBagIcon class="size-6 shrink-0 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
                  <span v-if="itemCount > 0" class="absolute -top-1 -right-1 inline-flex items-center justify-center rounded-full bg-red-600 text-white text-xs font-semibold leading-none h-5 min-w-[18px] px-1.5">{{ itemCount }}</span>
                  <span class="sr-only">items in cart, view bag</span>
                </a>
                <HeaderStackedShoppingCart :open="cartOpen" @close="cartOpen = false" />
              </div>
            </div>
          </div>
        </div>
